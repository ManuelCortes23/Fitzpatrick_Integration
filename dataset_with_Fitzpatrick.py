...
def get_df(kernel_type, out_dim, data_dir, data_folder, use_meta):
    ...
    #***********************************************************************
    # Fitzpatrick data 
    fp_df = pd.read_csv(os.path.join(data_dir, f'fitzpatrick_data', 'train.csv'))
    df_train3 = pd.DataFrame(columns = df_train.columns)

    df_train3.image_name = fp_df.md5hash
    df_train3.diagnosis = fp_df.three_partition_label

    df_train3.diagnosis[df_train3.diagnosis == 'malignant'] = 'melanoma'
    df_train3.benign_malignant = 'benign' #.fillna('benign')
    df_train3.benign_malignant[df_train3.diagnosis == 'malignant'] = 'malignant' #.fillna('benign')
    df_train3.target[df_train3.benign_malignant == 'benign'] = 0
    df_train3.target[df_train3.benign_malignant == 'malignant'] = 1
    df_train3['filepath'] = df_train3['image_name'].apply(lambda x: os.path.join(data_dir, f'fitzpatrick_data/train', f'resized_{x}.jpg'))
    df_train3.width = df_train3.height = 512
    df_train3.fold = np.random.randint(0 ,5, df_train3.shape[0]   )
    df_train3['is_ext'] = 1
    #***********************************************************************
    ...
    # concat train data
    # val = input("Enter 1 for original data \nEnter 2 to append fitzpatrick\nEnter 3 for testing FP integration\n ")
    
    val = 2
    
    if val == 2:
        df_train = pd.concat([df_train, df_train2, df_train3]).reset_index(drop=True)
    elif val == 3:
        df_train = df_train3.reset_index(drop=True)
    else:
